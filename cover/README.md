...
| `Typography` | The `androidx.compose.material3.Typography` Cover's `MaterialTheme` is built with. Display, headline, title and label styles are set in `PixelifySans`; body styles are set in `Handjet` (sized a step above Material's defaults, since its glyphs run small). Both are bundled variable fonts under `res/font`, licensed SIL OFL (`licenses/`). Variable-font weights need API 26+, so on API 24-25 every weight renders as Regular. |
